...
public static class StageManager
{
    private struct Zone
    {
        ...
        public Zone(float cx, float cy, float hx, float hy)
        {
            center = new Vector2(cx, cy);
            halfExtent = new Vector2(hx, hy);
        }
    }

    private static readonly Zone hub = new Zone(-200f, -100f, 9.5f, 9.5f);

    // 각 스테이지 구역의 실제 타일 범위에 맞춘 중심/절반 크기 (타일맵 실측값 기준, 여유 0.5 뺌).
    private static readonly Zone[] stages =
    {
        new Zone(-200f,   -72.5f, 10.5f, 9.0f),
        new Zone(-170.5f, -72f,   11.0f, 9.5f),
        new Zone(-138.5f, -100.5f, 10.0f, 9.0f),
        new Zone(-138.5f, -72f,   10.0f, 9.5f),
        new Zone(-112.5f, -99.5f, 11.0f, 10.0f),
        new Zone(-112.5f, -72f,   11.0f, 9.5f),
        new Zone(-84.5f,  -99.5f, 11.0f, 10.0f),
        new Zone(-84.5f,  -72f,   11.0f, 9.5f),
    };
    ...
    private static void MoveTo(Vector2 center, Vector2 halfExtent)
    {
        GameObject player = GameObject.Find("Player");
        if (player != null)
        {
    ...
    private static void SetInStage(bool inStage, Vector2 center, Vector2 halfExtent)
    {
